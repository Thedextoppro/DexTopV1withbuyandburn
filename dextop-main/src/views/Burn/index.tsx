...
const Burn: React.FC<React.PropsWithChildren> = () => {
  ...
  return (
    <Page>
          ...
          <CardWrapper>
            <Card p="0px" style={{ zIndex: 1 }}>
              <StyledCardBody style={{ padding: '18px 24px' }}>
                ...
                <Text bold fontSize={16}>Bounty: {formatAmount(estBounty)} DEX/~$0.00</Text>
              </StyledCardBody>
              <StyledCardFooter>
                {!account ?
                  <Box>
                  ...
                  <Box>
                    <Button width={"100%"} onClick={async() => await onBurn()}>Burn</Button>
                  </Box>}
              </StyledCardFooter>
            </Card>
          </CardWrapper>
      ...
      <BurnTable burnLPDatas={data.data.lps} setSelectedUsd={setSelectedUsd} setEstBounty={setEstBounty} setEstBurnDex={setEstBurnDex} setToken0s={setToken0s} setToken1s={setToken1s} dexPrice={data.data.dexPrice} />
    </Page>
  )
}
